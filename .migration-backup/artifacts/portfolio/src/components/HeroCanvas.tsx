...
// ─── Popup content per node (null = no popup on click) ───────────────────────
...
function NetworkScene({ mouse, scrollImpulse, onDismiss }: SceneProps) {
  ...
  useFrame(({ clock }) => {
    ...
    // Scroll impulse burst
    const imp = scrollImpulse.current;
    if (imp > 0.005) {
      for (let i = 0; i < NODE_COUNT; i++) {
        vel.current[i*3]   += (Math.random() - 0.5) * imp * 0.25;
        vel.current[i*3+1] += (Math.random() - 0.5) * imp * 0.25;
        vel.current[i*3+2] += (Math.random() - 0.5) * imp * 0.12;
      }
        ...
        return (
          <mesh
            ...
            onPointerOver={e => {
              e.stopPropagation();
              setHovered(i);
              document.body.style.cursor = content ? 'pointer' : 'default';
            }}
            onPointerOut={e => {
              e.stopPropagation();
              setHovered(null);
            ...
            onPointerDown={e => {
              e.stopPropagation();
              if (content) setSelected(isSel ? null : i);
            }}
          >
            <sphereGeometry args={[r, 10, 10]} />
            <meshBasicMaterial
              color={isHub ? '#fb923c' : '#f97316'}
              transparent
              opacity={0.65}
            />
              ...
              >
                <div
                  style={{
                    ...
                    background: 'hsl(20 8% 7% / 0.97)',
                    border: '1px solid hsl(32 98% 54% / 0.45)',
                    borderRadius: '3px',
                    padding: '14px 18px',
                    minWidth: '190px',
                    maxWidth: '230px',
                    boxShadow: '0 0 28px hsl(32 98% 54% / 0.18)',
                  }}
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                    ...
                    <button
                      ...
                      style={{ background: 'none', border: 'none', color: 'hsl(0 0% 45%)', cursor: 'pointer', fontSize: '16px', lineHeight: 1, padding: 0, marginTop: '-3px', flexShrink: 0 }}
                      aria-label="Close"
                    >
                      ×
                    </button>
...
export function HeroCanvas() {
  ...
  const onScroll = useCallback(() => {
    scrollImpulse.current = Math.min(scrollImpulse.current + 0.12, 0.35);
  }, []);
