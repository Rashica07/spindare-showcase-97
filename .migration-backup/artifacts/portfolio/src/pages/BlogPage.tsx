...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  "Architettura": "text-purple-400 border-purple-400/30 bg-purple-400/10",
  "Dizajn": "text-pink-400 border-pink-400/30 bg-pink-400/10",
};
...
export default function BlogPage() {
  ...
  const filtered = active === t.blog.categories[0]
    ? t.blog.posts
    : t.blog.posts.filter((p) => p.category === active || t.blog.categories.includes(p.category));
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="py-16" data-testid="blog-grid">
        <div className="max-w-7xl mx-auto px-6">
              ...
              <button
                ...
                className={`font-mono text-xs tracking-widest uppercase px-4 py-2 rounded-lg border transition-all duration-200 ${
                  active === cat
                    ...
                    : "text-muted-foreground border-border/60 hover:text-foreground hover:border-muted-foreground/60 bg-card/40"
                }`}
              >
                {cat}
              </button>
            ...
            {filtered.map((post, i) => {
              ...
              return (
                <FadeUp key={post.slug} delay={i * 0.08}>
                  ...
                  <motion.article
                    whileHover={{ y: -6, borderColor: "hsl(var(--primary) / 0.35)", boxShadow: "0 10px 30px -10px hsl(var(--primary) / 0.1)" }}
                    transition={{ duration: 0.2 }}
                    className="group border border-card-border bg-card/70 backdrop-blur-sm rounded-xl p-8 flex flex-col gap-5 transition-all cursor-pointer"
          ...
          {filtered.length === 0 && (
            <div className="py-20 text-center text-muted-foreground text-sm" data-testid="blog-empty">
