...
export default function HomePage() {
  ...
  // States for sticky process timeline
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden" data-testid="section-hero">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-6 pt-24 pb-16 w-full">
          ...
          <motion.h1
            ...
            className="text-5xl md:text-7xl lg:text-8xl font-bold leading-[1.02] tracking-tight max-w-5xl"
          ...
          >
            <Link href="/contact" data-testid="hero-cta-primary">
              <motion.span
                ...
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center gap-2 px-6 py-3.5 bg-primary text-primary-foreground text-sm font-semibold rounded glow-orange-sm hover:bg-primary/90 transition-all cursor-pointer"
              >
                {t.hero.cta1}
                <ArrowRight size={16} />
              </motion.span>
            ...
            <Link href="/portfolio" data-testid="hero-cta-secondary">
              <motion.span
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center gap-2 px-6 py-3.5 border border-border/60 text-foreground text-sm font-medium rounded-lg hover:bg-card transition-all cursor-pointer"
              >
                {t.hero.cta2}
      ...
      <section className="py-20 border-t border-border/40" data-testid="section-services">
        <div className="max-w-7xl mx-auto px-6">
              ...
              <FadeUp key={i} delay={i * 0.06}>
                <Link href="/services">
                  <motion.div
                    ...
                    className="group flex items-center justify-between gap-6 py-6 cursor-pointer"
                    data-testid={`service-row-${i}`}
                  >
                    <div className="flex items-center gap-6">
                      <span className="font-mono text-xs text-muted-foreground/30 w-6 shrink-0 select-none">
          ...
          <FadeUp className="mt-6 md:hidden">
            <Link href="/services">
              <motion.span
                whileHover={{ x: 4 }}
                className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors cursor-pointer"
        ...
        <div className="max-w-7xl mx-auto px-6">
            ...
            {t.work.projects.map((project, i) => (
              <FadeUp key={i} delay={i * 0.1}>
                <motion.div
                  ...
                  className="group border border-card-border bg-card rounded-xl p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-primary/30 transition-all duration-300 cursor-pointer"
                  data-testid={`project-card-${i}`}
                >
                  <div className="flex items-start gap-6">
                    <div className="font-mono text-4xl font-black text-muted-foreground/20 group-hover:text-primary/30 transition-colors leading-none select-none w-16">
                      {String(i + 1).padStart(2, "0")}
                    </div>
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`font-mono text-xs px-2 py-0.5 rounded-full border ${
                          project.status === "Live" || project.status === "In Development" && i === 0
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {project.stack.map((s, j) => (
                          <span key={j} className="font-mono text-xs text-muted-foreground border border-border/50 rounded px-2 py-0.5">
                            {s}
                          </span>
                        ))}
                      </div>
          ...
          <FadeUp className="mt-8 text-center">
            <Link href="/portfolio" data-testid="work-view-all">
              <motion.span
                whileHover={{ scale: 1.02 }}
                className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors cursor-pointer border border-border/60 rounded-lg px-5 py-3"
              >
      ...
      <section
        ref={timelineRef}
        className="timeline-sticky-section relative h-[300vh] border-t border-border/40 bg-card/10"
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {t.process.steps.map((step, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`timeline-step glass-card rounded-xl p-6 flex flex-col gap-4 relative h-full ${stepState}`}
                    data-testid={`process-step-${i}`}
                  >
                    <span className={`timeline-step-number font-mono text-3xl font-black transition-colors duration-300 ${isActive ? "" : "text-muted-foreground/30"}`}>
                      {step.n}
                    </span>
                    <h3 className={`timeline-step-title font-semibold transition-colors duration-300 ${isActive ? "" : "text-muted-foreground"}`}>
                      {step.title}
                    </h3>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="relative glass-card rounded-xl p-8 md:p-10 min-h-[300px] overflow-hidden flex flex-col justify-center">
            <div className="absolute right-6 top-6 flex gap-2 z-20">
              <button
                onClick={prevBlog}
                className="w-10 h-10 rounded-lg border border-border/60 flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-primary/40 transition-all cursor-pointer bg-card/60 backdrop-blur-sm"
              ...
              {blogPosts && blogPosts.length > 0 && (
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    <span className="font-mono text-xs px-2.5 py-1 rounded-full border border-primary/30 text-primary bg-primary/10">
                  ...
                  <div className="mt-8">
                    <Link href={`/blog/${blogPosts[blogIndex].slug}`}>
                      <motion.span
                        whileHover={{ x: 4 }}
                        className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary cursor-pointer"
                      >
      ...
      {/* CTA — links to dedicated contact page */}
              ...
              <div className="mt-10 flex justify-center">
                <Link href="/contact" data-testid="hero-cta-funnel-btn">
                  <motion.span
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-primary-foreground font-semibold rounded-lg text-sm tracking-wide hover:bg-primary/90 transition-all glow-orange-sm cursor-pointer shadow-lg"
                  >
                    {t.hero.cta1}
                    <ArrowRight size={16} />
                  </motion.span>
