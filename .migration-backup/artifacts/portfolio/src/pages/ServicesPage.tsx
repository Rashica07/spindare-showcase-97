          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {t.services.items.map((svc, i) => (
              <FadeUp key={i} delay={i * 0.1}>
                 <motion.div
                  whileHover={{ y: -6, borderColor: "hsl(var(--primary) / 0.35)", boxShadow: "0 10px 30px -10px hsl(var(--primary) / 0.12)" }}
                  transition={{ duration: 0.2 }}
                  className="border border-card-border bg-card/70 backdrop-blur-sm rounded-xl p-8 flex flex-col gap-6 transition-all"
                ...
                >
                  ...
                  <Link href="/contact">
                    <motion.span
                      ...
                      whileTap={{ scale: 0.98 }}
                      className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary cursor-pointer border border-primary/30 rounded-lg px-5 py-2.5 hover:bg-primary/10 transition-colors self-start"
                      data-testid={`service-detail-cta-${i}`}
                    >
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {t.process.steps.map((step, i) => (
              <FadeUp key={i} delay={i * 0.08}>
                <motion.div
                  whileHover={{ y: -4, borderColor: "hsl(var(--primary) / 0.25)" }}
                  transition={{ duration: 0.2 }}
