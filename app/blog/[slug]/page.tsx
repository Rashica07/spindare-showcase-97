...
function renderContent(content: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("## ")) {
      ...
      elements.push(
        <pre key={key++} className="blog-post-code">
          <code>{codeLines.join("\n")}</code>
        </pre>
      );
      i++;
        ...
        <p key={key++} className="blog-post-p">
          {parts.map((part, pi) => {
            if (part.startsWith("BOLD_START") && part.endsWith("BOLD_END")) {
              return <strong key={pi}>{part.slice(10, -8)}</strong>;
            }
            if (part.startsWith("CODE_START") && part.endsWith("CODE_END")) {
              return <code key={pi} className="blog-post-inline-code">{part.slice(10, -8)}</code>;
            }
            return part;
          })}
        </p>
  ...
  return (
    <>
      <section className="blog-post-hero">
        <div className="section-inner">
          <Link href="/blog" className="blog-post-back">
            <ArrowLeft size={14} /> {t.blog.hero.label}
          </Link>
      ...
      <article className="blog-post-body">
        <div className="section-inner blog-post-content">
