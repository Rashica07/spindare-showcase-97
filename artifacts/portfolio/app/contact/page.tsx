      ...
      <section className="py-20" data-testid="contact-body">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-5 gap-16">
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            {contactMeta.map(({ icon: Icon, label, value, href }, i) => (
              <FadeUp key={i} delay={i * 0.08}>
                <motion.div whileHover={{ y: -4, borderColor: "hsl(var(--primary) / 0.35)", boxShadow: "0 8px 24px -10px hsl(var(--primary) / 0.12)" }} transition={{ duration: 0.2 }} className="flex items-start gap-4 border border-card-border bg-card/60 backdrop-blur-sm rounded-xl p-5 transition-colors" data-testid={`contact-info-${i}`}>
          ...
          <div className="lg:col-span-3">
            <FadeUp delay={0.1}>
              {submitted ? (
                <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="border border-primary/30 bg-primary/5 rounded-xl p-10 text-center" data-testid="contact-success">
                  <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center mx-auto mb-4"><Check size={24} className="text-primary" /></div>
                  ...
                  <button onClick={() => { setSubmitted(false); form.reset(); }} data-testid="button-send-another" className="mt-6 text-xs text-muted-foreground hover:text-foreground transition-colors border border-border/60 rounded-lg px-4 py-2">
                  ...
                  <form onSubmit={form.handleSubmit(onSubmit)} className="glass-card rounded-xl p-8 flex flex-col gap-6" data-testid="contact-form">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormField control={form.control} name="name" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input placeholder={t.contact.form.namePh} {...field} data-testid="input-contact-name" className="bg-background border-border/60 font-mono text-sm" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={form.control} name="email" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input placeholder={t.contact.form.emailPh} type="email" {...field} data-testid="input-contact-email" className="bg-background border-border/60 font-mono text-sm" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                    </div>
                    <FormField control={form.control} name="subject" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input placeholder={t.contact.form.subjectPh} {...field} data-testid="input-contact-subject" className="bg-background border-border/60 font-mono text-sm" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="message" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Textarea placeholder={t.contact.form.messagePh} {...field} data-testid="textarea-contact-message" rows={6} className="bg-background border-border/60 font-mono text-sm resize-none" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <motion.button type="submit" disabled={submitting} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} data-testid="button-submit-contact" className="w-full py-4 bg-primary text-primary-foreground font-semibold rounded-lg text-sm tracking-wide hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                      {submitting ? (
                        <><span className="w-4 h-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />{t.contact.form.sending}</>
                      ) : (
